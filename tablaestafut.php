...
<body role="document" ng-controller="TablaController2 as tabla"  ng-keydown="onKeyDown($event)" ng-keyup="onKeyUp($event)">
				...
				<table class="table table-hover">
			  			<thead>
							<tr>
								...
								<th>Gol</th><th>TAP</th><th>TD</th>
								<th>TI%</th><th>PAS</th><th>AS</th>
								<th>FA</th><th>FDL</th><th>BAR</th>
								<th>TA</th><th>TR</th>
							</tr>
						</thead>
						...
						<tbody>
							...
							<td ng-dblclick="suma(juga,'tiempo_juego',1);">{{juga.tiempo_juego}}</td>
                                <td ng-dblclick="suma(juga ,'Gol',1);">{{juga.Gol}}</td> 
                                <td ng-dblclick="suma(juga ,'tiros_a_puerta',1);">{{juga.tiros_a_puerta}}</td> 
                                <td ng-dblclick="suma(juga ,'tiros_desviados',1);">{{juga.tiros_desviados}}</td>
                                <td ng-dblclick="suma(juga ,'tiros_por',1);">{{juga.tiros_por}}</td> 
                                <td ng-dblclick="suma(juga ,'pases',1);">{{juga.pases}}</td> 
                                <td ng-dblclick="suma(juga ,'asistencias',1);">{{juga.asistencias}}</td> 
                                <td ng-dblclick="suma(juga ,'faltas',1);">{{juga.faltas}}</td> 
                                <td ng-dblclick="suma(juga ,'fuera_lugar',1);">{{juga.fuera_lugar}}</td> 
                                <td ng-dblclick="suma(juga ,'barridas',1);">{{juga.barridas}}</td> 
                                <td ng-dblclick="suma(juga ,'amarillas',1);">{{juga.amarillas}}</td> 
                                <td ng-dblclick="suma(juga ,'rojas',1);">{{juga.rojas}}</td>
...
{	
	    ...
	    try {
	        ...
	        $stmt = $dbh->prepare("SELECT * FROM torneo WHERE Deporte='Futbol' AND activo='1'");
